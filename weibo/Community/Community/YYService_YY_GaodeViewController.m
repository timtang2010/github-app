...
{
    //比较接收的字符串：是否显示用户位置
    if ([keyPath isEqualToString:@"ShowUserLocation"]) {
        //返回具有给定键关联的值 变化： 背景: 获取更多信息
        NSNumber *shouNum = [change objectForKey:NSKeyValueChangeNewKey];
        
        self.showSegment.selectedSegmentIndex = ![shouNum boolValue];
    }
}
...
- (void)returnAction
{
    ...
    [self.mapView removeObserver:self forKeyPath:@"showUserLocation"];
